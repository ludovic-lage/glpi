...
if ($data['itemtype']=='Ticket')	
	  $SELECT .= "`glpi_entities`.`completename` AS `ITEM_entity_cn`,";
//Fin ajout SDF
...
//          $headers_line_bottom .= self::showEndHeader($data['display_type']);
   ...
   **/
   static function showGenericSearch($itemtype, array $params) {
      ...
      if (!empty($p["bookname"])){
		echo "<div class='tooltip'>Marque page actif !";
		echo "<span class='tooltiptext'>".$p['bookname']."</span>";
		echo "</div>";
      }
      //echo "Marque Page actif : ".$p["bookname"];
         ...
         switch ($table.'.'.$field) {
			...
			//Ajout SDF
				if ($itemtype == 'Ticket') {
						$out = "";
						$tooltip = "";
						$entite = Entity::getEntityByName($data['entity'][0]['cn']);
						$commentaire = "";
						$commentaire .= "<strong>Téléphone : </strong><a href=\"callto:".$entite->fields['phonenumber']."\">".$entite->fields['phonenumber']."</a><br/>";
						$commentaire .= $entite->fields['address']."<br/>";
						$commentaire .= $entite->fields['postcode']."&nbsp;".$entite->fields['town']."<br/>";
						$commentaire .= "<strong>Produits : </strong>Non connu<br/>";
						$commentaire .= "<strong>Commentaires : </strong>".$entite->fields['comment']."<br/>";
						$tooltip = Html::showToolTip($commentaire, array('display' => false));
						$out .= sprintf(__('%1$s %2$s'), $data['entity'][0]['cn'], $tooltip);
						return $out;
			   }
			   //Fin ajout SDF
               if ($itemtype == 'User') {
